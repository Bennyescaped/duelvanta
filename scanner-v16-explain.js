(()=>{
  ...
  function variantOf(c,tcg){return c?.v16VariantFamily||root.DV_SCAN_V16_QUALITY?.variantFamily?.(c,tcg)||null}
  ...
  function headlineFor(result,{candidateIndex=0}={}){
    ...
    if(candidateIndex>0)return{headline:'MANUELL AUSGEWÄHLT · BITTE PRÜFEN',tone:'review',summary:'Du hast einen anderen Kandidaten als den automatischen Top-Treffer gewählt. Die ursprüngliche Confidence gilt dafür nicht unverändert.'};
  ...
  function decorate(){if(typeof document==='undefined')return;addStyles();const host=document.getElementById('dvV16Results');if(!host)return;const cards=[...host.querySelectorAll('.dvV16Result')];cards.forEach((card,i)=>{const r=state.results[i];if(!r)return;const select=card.querySelector('[data-v16-select]'),idx=Math.max(0,Number(select?.value||0)),data=explain(r,r.v16ExplainTcg||'pokemon',{candidateIndex:idx});let panel=card.querySelector('.dvV16Explain');if(!panel){card.insertAdjacentHTML('beforeend',panelHtml(data));panel=card.querySelector('.dvV16Explain')}else panel.outerHTML=panelHtml(data);if(select&&!select.dataset.v16ExplainBound){select.dataset.v16ExplainBound='1';select.addEventListener('change',()=>setTimeout(decorate,0))}})}
