(()=>{
    ...
    if(!document.getElementById('dvSealedDialog')){
      ...
      d.innerHTML=`<div class="modal" style="max-width:760px"><div class="modalHead"><h2 id="dvSealedHeading">Sealed anbieten</h2><button class="close" type="button" data-dv-sealed-close>✕</button></div>
        <div class="dvSealedGrid">
          ...
          <div class="field"><label>Angebotsart</label><select id="dvSListingType"><option value="sale">Verkauf</option></select></div>
          <div class="field"><label>Preisart</label><select id="dvSPricingMode"><option value="fixed">Festpreis · sofort kaufen</option><option value="negotiable">Verhandlungsbasis · Angebote</option></select></div>
  ...
  function refreshFormRules(){
    ...
    if(tradeOnly){e.pricingMode.value='negotiable';e.pricingMode.disabled=true}else e.pricingMode.disabled=false;
  }
  function resetForm(){
    const e=formEls();
    e.tcg.value='pokemon';e.cat.value='display';e.name.value='';e.set.value='';e.lang.value='Deutsch';e.cond.value='factory_sealed';e.qty.value='1';e.minQty.value='1';e.tier1Qty.value='';e.tier1Price.value='';e.tier2Qty.value='';e.tier2Price.value='';e.units.value='';e.contents.value='';e.weight.value='';e.l.value='';e.w.value='';e.h.value='';e.listingType.value='sale';e.pricingMode.value='fixed';e.price.value='';e.shipping.value='parcel';e.shippingCost.value='';e.shippingNote.value='';e.note.value='';e.files.value='';e.photos.innerHTML='';e.msg.textContent='';
    refreshFormRules();
  }
  function fillForm(l){
    const e=formEls();
    const tiers=Array.isArray(l.quantity_pricing)?l.quantity_pricing:[];e.tcg.value=l.tcg||'pokemon';e.cat.value=l.sealed_category||'display';e.name.value=l.card_name||'';e.set.value=l.set_name||'';e.lang.value=l.language||'';e.cond.value=l.sealed_condition||'factory_sealed';e.qty.value=l.quantity_available??l.stock_quantity??l.bundle_quantity??1;e.minQty.value=l.minimum_purchase_quantity||1;e.tier1Qty.value=tiers[0]?.min_quantity||'';e.tier1Price.value=tiers[0]?.unit_price||'';e.tier2Qty.value=tiers[1]?.min_quantity||'';e.tier2Price.value=tiers[1]?.unit_price||'';e.units.value=l.units_per_container||'';e.contents.value=l.package_contents||'';e.weight.value=l.weight_grams||'';e.l.value=l.length_mm||'';e.w.value=l.width_mm||'';e.h.value=l.height_mm||'';e.listingType.value='sale';e.pricingMode.value=l.pricing_mode||'negotiable';e.price.value=l.asking_price??'';e.shipping.value=l.shipping_method||'parcel';e.shippingCost.value=l.shipping_cost??'';e.shippingNote.value=l.shipping_note||'';e.note.value=l.seller_note||'';e.files.value='';e.msg.textContent='';
    refreshFormRules();
  }
  function validateForm(existingCount=0){
    ...
    if(['sale','sale_or_trade'].includes(e.listingType.value)&&(!price||price<=0))throw new Error('Für Verkauf bitte einen positiven Preis angeben.');
    ...
    for(const t of tiers)if(!Number.isInteger(t.min_quantity)||t.min_quantity<=minQty||t.min_quantity>(qty+(mode==='edit'?Number(editListing.stock_quantity)-Number(editListing.quantity_available):0))||!t.unit_price||t.unit_price<=0||t.unit_price>=price)throw new Error('Mengenpreise brauchen eine gültige Menge und einen Stückpreis unter dem Basispreis.');
  ...
  function rpcPayload(v){
    ...
    return {
      ...
      p_stock_quantity:v.qty+(mode==='edit'?Number(editListing.stock_quantity)-Number(editListing.quantity_available):0),p_minimum_purchase_quantity:v.minQty,p_quantity_pricing:v.tiers,p_package_contents:e.contents.value.trim()||null,p_units_per_container:numberOrNull(e.units.value),p_weight_grams:numberOrNull(e.weight.value),p_length_mm:numberOrNull(e.l.value),p_width_mm:numberOrNull(e.w.value),p_height_mm:numberOrNull(e.h.value),
      p_listing_type:'sale',p_pricing_mode:e.pricingMode.value,p_asking_price:v.price,p_seller_note:e.note.value.trim()||null,p_shipping_method:e.shipping.value,p_shipping_cost:v.ship,p_shipping_note:e.shippingNote.value.trim()||null
    };
  }
  ...
  async function saveSealed(){
    ...
    try{
      ...
      if(mode==='create'){
        const {data,error}=await db.rpc('create_sealed_market_listing_draft_v3',payload);if(error)throw error;draftId=data;
        ...
        const {error}=await db.rpc('edit_my_sealed_market_listing_v3',editPayload);if(error)throw error;
  ...
  function install(){
    ...
    const e=formEls();e.cat.onchange=refreshFormRules;e.shipping.onchange=refreshFormRules;e.listingType.onchange=refreshFormRules;e.save.onclick=saveSealed;
    const offerQtyBox=document.getElementById('dvOfferQuantity'),offerQty=document.getElementById('dvOfferQty'),offerHint=document.getElementById('dvOfferQtyHint'),sendOffer=document.getElementById('sendOffer'),amountInput=document.getElementById('offerAmount'),originalSendOffer=sendOffer?.onclick;
    let offerReview=null,offerReviewTimer=null,offerReviewSeq=0,offerReviewBox=document.getElementById('dvOfferContractReview');
    if(!offerReviewBox&&sendOffer){offerReviewBox=document.createElement('div');offerReviewBox.id='dvOfferContractReview';offerReviewBox.className='dvSealedHint';offerReviewBox.style.marginTop='12px';sendOffer.insertAdjacentElement('beforebegin',offerReviewBox)}
    const offerMoney=n=>Number(n||0).toLocaleString('de-DE',{style:'currency',currency:'EUR'});
    function clearOfferReview(text='Vertragsdaten werden nach Menge und Preis serverseitig geprüft.'){offerReview=null;if(sendOffer)sendOffer.disabled=true;if(offerReviewBox)offerReviewBox.textContent=text}
    function renderOfferReview(r){const s=r?.seller_party||{},party=s.business_name||s.legal_name||'Verkäufer',method=METHODS[r.shipping_method]||r.shipping_method||'Versand';offerReviewBox.innerHTML='<b>VERBINDLICHES PREISANGEBOT</b><br>Vertragspartner: '+htmlEscape(party)+' · '+htmlEscape(s.role_label||'Verkäufer')+'<br>'+htmlEscape(r.product?.title||'Produkt')+' · '+Number(r.quantity||1)+' Stück<br>Produktangebot: '+offerMoney(r.offered_goods_total)+' · '+htmlEscape(method)+': '+offerMoney(r.shipping_cost)+'<br><strong>GESAMTPREIS: '+offerMoney(r.total_price)+'</strong><br><span>'+htmlEscape(r.offer_notice||'Der Vertrag entsteht erst durch Annahme des Verkäufers.')+'</span>'}
    function offerInputs(){const l=typeof offerTarget!=='undefined'?offerTarget:null,isSealed=l?.product_kind==='sealed',qty=isSealed?Number(offerQty?.value):1,amount=Number(amountInput?.value),available=isSealed?Number(l?.quantity_available??l?.stock_quantity??l?.bundle_quantity??0):1,min=isSealed?Number(l?.minimum_purchase_quantity||1):1;return{l,qty,amount,valid:!!l&&Number.isInteger(qty)&&qty>=min&&qty<=available&&Number.isFinite(amount)&&amount>0}}
    function scheduleOfferReview(){clearTimeout(offerReviewTimer);const seq=++offerReviewSeq,{l,qty,amount,valid}=offerInputs();clearOfferReview(valid?'Verkäufer, Ware, Versand und Gesamtpreis werden geprüft …':'Bitte gültige Menge und Angebotssumme eingeben.');if(!valid||window.DV_TRADE_LEGAL_SCHEMA?.available!==true)return;offerReviewTimer=setTimeout(async()=>{const{data,error}=await db.rpc('review_market_price_offer_v1',{p_listing_id:l.id,p_requested_quantity:qty,p_amount:amount});if(seq!==offerReviewSeq||offerTarget?.id!==l.id)return;if(error||!data||data.snapshot_version!=='price-offer-contract-v1'||!data.offer_review_hash){clearOfferReview(error?.message||'Vertragsdaten konnten nicht geprüft werden.');return}offerReview=data;renderOfferReview(data);sendOffer.disabled=false},80)}
    function refreshOfferQuantity(commit=false){const l=typeof offerTarget!=='undefined'?offerTarget:null,isSealed=l?.product_kind==='sealed';if(!offerQtyBox)return;offerQtyBox.style.display=isSealed?'':'none';if(!isSealed){scheduleOfferReview();return}const available=Number(l.quantity_available??l.stock_quantity??l.bundle_quantity??1),min=Number(l.minimum_purchase_quantity||1),raw=offerQty.value.trim();offerQty.min=String(min);offerQty.max=String(available);if(raw===''){offerHint.textContent=`${available} verfügbar · Menge zwischen ${min} und ${available} eingeben.`;if(commit){offerQty.value=String(min);refreshOfferQuantity(false)}else clearOfferReview();return}const entered=Number(raw);if(!Number.isInteger(entered)||entered<min||entered>available){offerHint.textContent=`Bitte eine Menge zwischen ${min} und ${available} eingeben.`;if(commit){offerQty.value=String(Math.max(min,Math.min(available,Number.isFinite(entered)?Math.round(entered):min)));refreshOfferQuantity(false)}else clearOfferReview();return}const unit=unitPriceFor(l,entered),total=unit*entered;offerHint.textContent=`${available} verfügbar · Richtpreis ${total.toLocaleString('de-DE',{style:'currency',currency:'EUR'})} (${unit.toLocaleString('de-DE',{style:'currency',currency:'EUR'})}/Stück)`;amountInput.value=total.toFixed(2);scheduleOfferReview()}
    offerQty?.addEventListener('input',()=>refreshOfferQuantity(false));offerQty?.addEventListener('blur',()=>refreshOfferQuantity(true));amountInput?.addEventListener('input',scheduleOfferReview);
    grid.addEventListener('click',ev=>{const btn=ev.target.closest('[data-offer]');if(!btn)return;clearOfferReview();setTimeout(()=>{const l=typeof offerTarget!=='undefined'?offerTarget:null;offerQty.value=String(Number(l?.minimum_purchase_quantity||1));refreshOfferQuantity(false)},0)},true);
    if(sendOffer)sendOffer.onclick=async()=>{const{l,qty,amount,valid}=offerInputs();if(!l)return originalSendOffer?.call(sendOffer);if(!valid||!offerReview||sendOffer.disabled){document.getElementById('offerMsg').textContent='Bitte warte auf die vollständige Vertragsprüfung.';return}sendOffer.disabled=true;const{error}=await db.rpc('create_market_offer_v3',{p_listing_id:l.id,p_requested_quantity:qty,p_amount:amount,p_message:document.getElementById('offerMessage').value.trim()||null,p_expected_updated_at:offerReview.listing_updated_at,p_offer_review_hash:offerReview.offer_review_hash});document.getElementById('offerMsg').textContent=error?(String(error.message||'').includes('offer_review_changed')?'Inserat oder Vertragsdaten haben sich geändert. Bitte erneut prüfen.':error.message):'Verbindliches Preisangebot gesendet.';if(!error){offerReview=null;setTimeout(()=>document.getElementById('offerDialog').close(),700);await loadListings()}else scheduleOfferReview()};
    e.files.onchange=()=>{
    ...
    scheduleDecorate();window.DV_TRADE_SEALED={version:'1.2.1',refresh:scheduleDecorate,openCreate};return true;
  }
