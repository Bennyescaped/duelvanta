...
(() => {
  ...
  function note(s){
    ...
    if(s.technical_completed_at)bits.push(`Technisch abgeschlossen: ${fmt(s.technical_completed_at)} · ${esc(s.technical_completion_reason||'')}`);
    return bits.join('<br>');
  }
  ...
  function decorate(){
    document.querySelectorAll('.dvOrderCard').forEach(card=>{
      ...
      if(!s)return;
      const signature=[s.status,s.tracking_required,s.shipping_due_at||'',s.delivery_evidence_at||'',s.closure_eligible_at||'',s.pickup_code_pending,s.pickup_code_expires_at||'',s.active_case_due_at||'',s.technical_completed_at||'',s.technical_completion_reason||''].join('|');
      ...
      if(s.fulfillment_group==='pickup'&&s.pickup_code_pending&&!pickup){
        ...
        pickup.type='button';
        pickup.className='btn gold';
        pickup.dataset.b07PickupConfirm=id;
        pickup.textContent='ÜBERGABECODE BESTÄTIGEN';
  ...
  function dialog(){
    ...
    document.getElementById('dvB07LifeClose').onclick=()=>d.close();
    return d;
  }

  async function sellerPickup(id){
    const d=dialog();
    document.getElementById('dvB07LifeTitle').textContent='Persönliche Übergabe';
    document.getElementById('dvB07LifeBody').innerHTML='<div class="msg">Einmaliger Übergabecode wird erzeugt …</div>';
    document.getElementById('dvB07LifeMsg').textContent='';
    d.showModal();
  ...
  async function buyerPickup(id){
    const code=prompt('Einmaligen Übergabecode des Verkäufers eingeben:');
    if(!code)return;
    const {data,error}=await db.rpc('confirm_market_pickup_handover_b07',{p_order_id:id,p_code:code.trim()});
    if(error){alert(error.message);return}
    if(!data?.ok){alert('Übergabecode ist ungültig.');return}
    await load();
  ...
  function enforceDialogHints(target){
    ...
    if(!s)return;
    setTimeout(()=>{
      const country=document.getElementById('oCountry');
      if(country){[...country.options].forEach(o=>{if(o.value!=='DE')o.remove()});country.value='DE'}
      ...
      if(target.dataset.oQuote&&s.tracking_required){
        const method=document.getElementById('oMethod');
        if(method){[...method.options].forEach(o=>{if(!['tracked_letter','parcel'].includes(o.value))o.remove()});if(!['tracked_letter','parcel'].includes(method.value))method.value='tracked_letter'}
  ...
  function mutationAddsOrderCard(mutation){
    return [...mutation.addedNodes].some(node=>node.nodeType===1&&(node.matches?.('.dvOrderCard')||node.querySelector?.('.dvOrderCard')));
  }
  ...
  function install(){
    ...
    const observer=new MutationObserver(mutations=>{
      if(!mutations.some(mutationAddsOrderCard))return;
      scheduleReload();
    });
  ...
  let tries=0;
  const wait=setInterval(()=>{tries++;if(install()||tries>150)clearInterval(wait)},80);
})();
