...
const {provider}=require('./ximilar-server.cjs');
...
function createHandler({env=process.env,config,fetchImpl=fetch,now=Date.now,callProvider=provider.call}={}){
  ...
  return async(req,res)=>{
    ...
    try{
      ...
      if(req.method==='GET'){
        ...
        return res.status(200).json({active:budget?.enabled===true,remaining:Math.max(0,Number(budget?.remaining)||0)});
      }
      ...
      requireThat(body&&Object.keys(body).sort().join(',')==='imageBase64,requestId,tcg'&&UUID.test(body.requestId),400,'invalid_request');
      ...
      const reservation=await json(AUTH_URL+'/rest/v1/rpc/dv_v16_reserve_scan',{method:'POST',headers,body:JSON.stringify({p_request_id:body.requestId,p_image_sha256:sha256,p_tcg:body.tcg})});
      requireThat(reservation?.allowed===true,reservation?.reason==='duplicate'?409:429,reservation?.reason==='duplicate'?'scan_already_reserved':'scan_limit_reached');
      const result=await callProvider({image,tcg:body.tcg,env,fetchImpl,now});
      const {raw,...compact}=result; // raw evidence belongs in the private pilot, not every mobile response.
      return res.status(200).json({...compact,selectedTcg:body.tcg,sha256,remaining:Math.max(0,Number(reservation.remaining)||0)});
