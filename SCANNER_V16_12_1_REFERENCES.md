...
- `scanner-v16-references.js` lädt die kleine Zuordnung einmal mit einem Zeitlimit. Erst **nach einer normalen Katalogsuche** ersetzt sie Bild-URLs durch mitgelieferte Dateien, wenn die gesamte zurückgegebene Kandidatengruppe abgedeckt ist. TCG und vollständige Original-URL müssen übereinstimmen. Bei einer fehlenden Variante nutzt die komplette Gruppe weiterhin den bisherigen Anbieterpfad. Gespeicherte und live geladene Bildstände werden innerhalb eines Rankings nicht gemischt.
...
- Core, Evidence-Gates, Importregeln, OCR, Live-Kamera und Benchmark-Speicher werden nicht geändert. Neben der Versionsanzeige kennzeichnet Explain eine erfolgreich manuell eingegebene Nummer ausdrücklich als manuelle Kandidatenprüfung statt als automatischen Treffer. COLLECT/V15.8, Auth/RLS, `main` und das Original-Logo bleiben unverändert.
...
Die erste CI-Ausführung des Caches stoppte bei P-001: Die Rangfolge zwischen Winner und Participant änderte sich beim Mischen einer gespeicherten Participant-Referenz mit live gelieferten Varianten. Die strikte Prüfung auf den exakten Druck wurde beibehalten. Daraus folgt die oben beschriebene allgemeine Regel für vollständige Kandidatengruppen; eine Einzelkarte wurde nicht als Ausnahme im Ranking behandelt. Im ersten Browsernachtest erhielt Ganzui B nun einen Artworkwert von 64 %, blieb wegen der einzelnen OCR-Lesung korrekt bei 74 % und „Prüfen“.
