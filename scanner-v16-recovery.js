(()=>{
  ...
  function canConfirm(row,card){
    ...
    return !!(row?.id&&card?.catalogVerified&&card.tcg===row.tcg&&api.candidateCode(card,row.tcg)===api.idCode(row.id,row.tcg));
  }
  async function search(row,code,analyze){
    ...
    const pack=await analyze(row.crop,{mode:'single',tcg:row.tcg,identifierOverride:id,sourcePrepared:true,manualRecovery:true});
    const result=pack.results?.[0];if(!result)throw new Error('Die Katalogsuche hat kein Ergebnis zurückgegeben.');
    return{...result,index:row.index,slot:row.slot,row:row.row,col:row.col,chosen:0,selected:false,manualRecovery:true,manualConfirmed:false};
  }
