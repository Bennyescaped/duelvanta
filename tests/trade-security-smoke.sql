...
begin
  ...
  foreach f in array array[
    ...
    'public.seed_market_order_default_shipping_address()',
  ...
  if exists(select 1 from pg_class where oid in (
    'public.market_listings'::regclass,'public.market_offers'::regclass,'public.market_deals'::regclass,
    'public.market_orders'::regclass,'public.market_order_items'::regclass,'public.market_notifications'::regclass,
    'public.market_default_shipping_addresses'::regclass,'public.market_shipping_profiles'::regclass,
    'public.market_shipping_profile_rules'::regclass
  ...
  foreach f in array array['public.market_notifications','public.market_default_shipping_addresses','public.market_shipping_profiles','public.market_shipping_profile_rules'] loop
    if has_table_privilege('anon',f,'select') or has_table_privilege('anon',f,'insert') or has_table_privilege('anon',f,'update') or has_table_privilege('anon',f,'delete')
       or has_table_privilege('authenticated',f,'select') or has_table_privilege('authenticated',f,'insert') or has_table_privilege('authenticated',f,'update') or has_table_privilege('authenticated',f,'delete') then
      raise exception 'Sensitive table must remain RPC-only: %',f;
    end if;
  end loop;
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='profiles' and column_name in ('street_line1','street_line2','postal_code','city','country_code','shipping_address')) then
    raise exception 'Private shipping address leaked into public profile table';
  end if;
  ...
  foreach call_sql in array array[
    ...
    'select public.get_my_default_shipping_address()',
    'select public.delete_my_default_shipping_address()',
    'select public.get_my_market_shipping_profiles()',
...
rollback;
select 'PASS: checkout, automation, private addresses, shipping profiles, RLS, auth guards and inventory invariants' as smoke_result;
