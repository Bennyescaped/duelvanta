...
try{
  await db.exec(`
    ...
    create table public.market_orders(id uuid,order_number text,item_count integer,total_amount numeric,buyer_id uuid,seller_id uuid,fulfillment_group text,status text,shipping_quote_status text,payment_provider text,payment_status text,shipped_at timestamptz,created_at timestamptz);
    ...
    insert into public.market_orders values
      ('${uid(20)}','ADDRESS',1,4,'${buyer}','${seller}','shipping','open','confirmed','manual_beta','paid',null,'2026-09-14'),
      ('${uid(21)}','SHIP',1,4,'${buyer}','${seller}','pickup','open','confirmed','manual_beta','paid',null,'2026-09-14'),
      ('${uid(22)}','QUOTE',1,4,'${buyer}','${seller}','pickup','open','review_required','stripe_connect','pending',null,'2026-09-14');
  ...
  const lifecycle=await readFile(new URL('../database/trade-order-lifecycle-payment-hardening-v1.sql',import.meta.url),'utf8');
  await db.exec(`
    insert into public.market_orders values
      ('${uid(23)}','RECEIVE-PAID',1,4,'${buyer}','${seller}','shipping','shipped','confirmed','stripe_connect','paid',now(),'2026-09-15'),
      ('${uid(24)}','RECEIVE-REFUNDED',1,4,'${buyer}','${seller}','shipping','shipped','confirmed','stripe_connect','refunded',now(),'2026-09-15');
  `);
  await db.exec(lifecycle);
  assert.deepEqual((await actions(buyer)).map(r=>[r.action_type,r.subject]),[
    ['add_address','ADDRESS'],
    ['confirm_received','RECEIVE-PAID']
  ]);
  assert.equal((await actions(buyer)).some(r=>r.subject==='RECEIVE-REFUNDED'),false);
  ...
  await db.exec(lifecycle); // final lifecycle hardening must remain repeatable after older upgrade scripts.
  await db.exec('set role anon');
  await assert.rejects(()=>actions(buyer),/permission denied/);
  await db.exec('reset role; set role authenticated');
  assert.equal((await actions(buyer)).some(r=>r.subject==='RECEIVE-REFUNDED'),false);
  assert.deepEqual(await actions(outsider),[]);
  console.log('PASS: real SQL action queries, ordering, ownership, anonymous denial and payment-gated lifecycle actions');
