...
finally{await db.close();await mkdir('test-results',{recursive:true});await writeFile(`test-results/account-data-export-${native?'native':'pglite'}${lock?'-lock':''}.json`,JSON.stringify(report,null,2));}
