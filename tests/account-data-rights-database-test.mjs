...
const db=new PGlite({extensions:{pgcrypto}}),USER='91000000-0000-4000-8000-000000000001',OTHER='91000000-0000-4000-8000-000000000002',CHAT='91000000-0000-4000-8000-000000000003';
const migrations=await Promise.all(['database/market-seller-compliance-v1.sql','database/market-notice-action-v1.sql','database/market-checkout-compliance-v1.sql','database/market-tax-transparency-v1.sql','database/account-data-rights-v1.sql','database/account-data-rights-pickup-messages-hardening-v1.sql'].map(name=>readFile(new URL('../'+name,import.meta.url),'utf8')));
...
try{
  await db.exec(`
    create role anon;create role authenticated;create role service_role;create schema auth;create schema extensions;create schema dv_market_private;create extension if not exists pgcrypto with schema extensions;
    ...
    create table public.market_orders(id uuid primary key default gen_random_uuid(),order_number text,seller_id uuid,buyer_id uuid,status text default 'completed',fulfillment_group text default 'shipping',payment_provider text default 'manual_beta',created_at timestamptz default now());
    ...
    create table dv_market_private.market_swap_threads(id uuid primary key default gen_random_uuid(),party_a_id uuid not null,party_b_id uuid not null,fulfillment_mode text not null default 'pickup',status text not null default 'completed');
    create table dv_market_private.market_pickup_messages(id uuid primary key default gen_random_uuid(),context_type text not null,context_id uuid not null,sender_id uuid not null,body text not null,created_at timestamptz default now());
    ...
    insert into auth.users values('${USER}','user@example.test'),('${OTHER}','other@example.test'),('${CHAT}','chat@example.test');
    insert into public.profiles(id,email,display_name,role,account_status) values('${USER}','user@example.test','User','player','active'),('${OTHER}','other@example.test','Other','player','active'),('${CHAT}','chat@example.test','Chat User','player','active');
  `);
  ...
  await db.exec(`
    ...
    insert into public.market_orders(id,order_number,seller_id,buyer_id,status,fulfillment_group) values('94000000-0000-4000-8000-000000000001','DV-EXPORT-PICKUP','${USER}','${OTHER}','completed','pickup');
    insert into dv_market_private.market_pickup_messages(context_type,context_id,sender_id,body,created_at) values
      ('order','94000000-0000-4000-8000-000000000001','${USER}','Meine Exportnachricht','2026-09-17T10:00:00Z'),
      ('order','94000000-0000-4000-8000-000000000001','${OTHER}','Antwort der Gegenseite','2026-09-17T10:01:00Z');
  `);
  await claim(USER);
  const exported=await value(`select public.export_my_duelvanta_data() value`);
  assert.equal(exported.export_version,'duelvanta-data-export-v2');assert.equal(exported.account.email,'user@example.test');assert.equal(exported.collection.items[0].notes,'eigene Notiz');
  assert.equal(exported.marketplace.tax_identifier_references[0].identifier_kind,'tin');
  assert.equal(exported.marketplace.pickup_messages.length,2);assert.deepEqual(exported.marketplace.pickup_messages.map(x=>x.sender_role),['self','other']);
  assert.equal(exported.marketplace.pickup_messages[0].body,'Meine Exportnachricht');
  ...
  await db.exec(`reset role;delete from dv_market_private.market_pickup_messages where context_id='94000000-0000-4000-8000-000000000001';`);
  ...
  await db.exec(`reset role;
    insert into public.market_orders(id,order_number,seller_id,buyer_id,status,fulfillment_group) values('94000000-0000-4000-8000-000000000003','DV-CHAT-HOLD','${CHAT}','${USER}','completed','pickup');
    insert into dv_market_private.market_pickup_messages(context_type,context_id,sender_id,body,created_at) values
      ('order','94000000-0000-4000-8000-000000000003','${CHAT}','Treffpunkt Chat-Nachricht','2026-09-17T11:00:00Z'),
      ('order','94000000-0000-4000-8000-000000000003','${USER}','Antwort bleibt Gegenpartei','2026-09-17T11:01:00Z');
  `);
  await claim(CHAT);const chatRequest=await value(`select public.request_my_account_deletion('KONTO LÖSCHEN','92000000-0000-4000-8000-000000000003') value`);assert.equal(chatRequest.accepted,true);
  await claim(null,'service_role');const chatClaim=(await db.query(`select * from public.claim_account_deletion_requests(5,'93000000-0000-4000-8000-000000000003')`)).rows[0];
  const chatPlan=await value(`select public.prepare_account_deletion_data('${chatClaim.request_id}','${chatClaim.delivery_lock_token}') value`);assert.equal(chatPlan.auth_action,'disable_and_retain');
  const chatHold=(await db.query(`select id,category,manual_review_required,retain_until from dv_market_private.account_deletion_holds where request_id='${chatClaim.request_id}' and category='pickup_messages'`)).rows[0];
  assert.equal(chatHold.category,'pickup_messages');assert.equal(chatHold.manual_review_required,true);assert.equal(chatHold.retain_until,null);
  await db.query(`select public.finish_account_deletion_request('${chatClaim.request_id}','${chatClaim.delivery_lock_token}',true,null)`);
  assert.equal((await db.query(`select status from dv_market_private.account_deletion_requests where id='${chatClaim.request_id}'`)).rows[0].status,'retained');
  await db.query(`select public.release_account_deletion_hold('${chatHold.id}','Externe Prüfung für diesen Test abgeschlossen')`);
  const chatClaim2=(await db.query(`select * from public.claim_account_deletion_requests(5,'93000000-0000-4000-8000-000000000004')`)).rows[0];
  const chatPlan2=await value(`select public.prepare_account_deletion_data('${chatClaim2.request_id}','${chatClaim2.delivery_lock_token}') value`);assert.equal(chatPlan2.auth_action,'delete');
  await db.query(`select public.finish_account_deletion_request('${chatClaim2.request_id}','${chatClaim2.delivery_lock_token}',true,null)`);
  assert.equal(Number((await db.query(`select count(*) count from dv_market_private.market_pickup_messages where sender_id='${CHAT}'`)).rows[0].count),0);
  assert.equal(Number((await db.query(`select count(*) count from dv_market_private.market_pickup_messages where context_id='94000000-0000-4000-8000-000000000003' and sender_id='${USER}'`)).rows[0].count),1);
  ...
  console.log('PASS: own-data export includes pickup messages, deletion blockers are enforced, pickup erasure uses a manual no-deadline hold, and erasure is staged with immutable audit');
