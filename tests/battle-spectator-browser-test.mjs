...
for(const [path,sha] of Object.entries({'battle.js':'9cb2d4368cde388d8c89fc44d88d0e707b7de13f','battle-webrtc.js':'5d97ae8e195264c6c14c18a33c4cbc5545533c14','site-nav.js':'345b4aeb7bc18a0e72df6e257200004748d8b6be'}))assert.equal(hash(await read(path)),sha,'protected player/runtime file: '+path);
...
async function pageFor(path='battle-spectator.html',init,noGuard=false){const page=await browser.newPage({viewport:{width:1280,height:900}});page.errors=[];page.on('pageerror',e=>page.errors.push(e.message));await page.route('**/*',route=>{const url=new URL(route.request().url());if(url.pathname==='/api/compliance-message-dispatch')return route.fulfill({contentType:'text/javascript',body:noGuard?'':'window.DV_SUPABASE=Object.freeze({url:"https://xhmjxrcskfhbovhitdej.supabase.co",key:"sb_publishable_test_only",environment:"preview"});'});if(url.hostname==='cdn.jsdelivr.net'){if(url.pathname.includes('livekit-client'))return route.fulfill({contentType:'text/javascript',body:'window.LivekitClient={};'});return route.fulfill({contentType:'text/javascript',body:mock});}if(url.origin===base)return route.continue();return route.abort();});if(init)await page.addInitScript(init);await page.goto(base+'/'+path);return page;}
