...
assert.equal(openai.accountingFrom({usage:{input_tokens:2000,input_tokens_details:{cached_tokens:1000},output_tokens:500}}).estimatedCostUsd,0.003075);
assert.equal(openai.accountingFrom({usage:{input_tokens:10,input_tokens_details:{cached_tokens:11},output_tokens:1}}).estimatedCostUsd,null);
await assert.rejects(openai.provider.call({image,tcg:'pokemon',env,now:()=>now,fetchImpl:async()=>({ok:true,json:async()=>({status:'incomplete',usage:{input_tokens:100,output_tokens:1400}})})}),e=>e.code==='provider_incomplete_or_blocked'&&e.accounting.estimatedCostUsd===0.006375);
assert.ok((400000*openai.PRICES_PER_MILLION.input+openai.MAX_OUTPUT_TOKENS*openai.PRICES_PER_MILLION.output)/1e6*0.862664<0.50,'reservation covers full supported context and bounded output');
