...
const eventOrdering=await readFile(new URL('../database/market-stripe-event-ordering-hardening-v1.sql',import.meta.url),'utf8');
...
try{
  ...
  await db.exec(eventOrdering);await db.exec(eventOrdering);
  ...
  await db.exec('set role service_role;');
  const lateCheckout=json((await db.query(`select public.apply_market_stripe_event('evt_TestLateCheckout','checkout.session.completed','acct_TestSeller',false,'cs_test_TestSession',repeat('e',64),now()-interval '1 hour','${JSON.stringify({attempt_id:prepared.attempt_id,payment_intent_id:'pi_TestIntent'})}'::jsonb) value`)).rows[0].value);
  assert.deepEqual({status:lateCheckout.status,note:lateCheckout.note},{status:'ignored',note:'checkout_state_preserved'});
  const repeatedSuccess=json((await db.query(`select public.apply_market_stripe_event('evt_TestPaidAgain','payment_intent.succeeded','acct_TestSeller',false,'pi_TestIntent',repeat('f',64),now()+interval '1 hour','${JSON.stringify({attempt_id:prepared.attempt_id,amount_cents:10500,currency:'eur',charge_id:'ch_TestCharge'})}'::jsonb) value`)).rows[0].value);
  assert.deepEqual({status:repeatedSuccess.status,note:repeatedSuccess.note},{status:'ignored',note:'payment_already_confirmed'});
  const lateFailure=json((await db.query(`select public.apply_market_stripe_event('evt_TestLateFailure','payment_intent.payment_failed','acct_TestSeller',false,'pi_TestIntent',repeat('1',64),now()-interval '2 hours','${JSON.stringify({attempt_id:prepared.attempt_id})}'::jsonb) value`)).rows[0].value);
  assert.deepEqual({status:lateFailure.status,note:lateFailure.note},{status:'ignored',note:'post_payment_state_preserved'});
  await assert.rejects(()=>db.query(`select public.apply_market_stripe_event('evt_TestWrongSession','checkout.session.completed','acct_TestSeller',false,'cs_test_OtherSession',repeat('2',64),now(),'${JSON.stringify({attempt_id:prepared.attempt_id,payment_intent_id:'pi_TestIntent'})}'::jsonb)`),/stripe_checkout_session_mismatch/);
  await db.exec('reset role;');
  assert.deepEqual((await db.query(`select state,paid_cents,refunded_cents from dv_market_private.market_payment_attempts where id='${prepared.attempt_id}'`)).rows[0],{state:'paid',paid_cents:10500,refunded_cents:0});
  assert.equal(Number((await db.query('select count(*) count from dv_market_private.market_tax_events')).rows[0].count),2);
  await db.exec('set role service_role;');
  ...
  const checkoutAfterRefund=json((await db.query(`select public.apply_market_stripe_event('evt_TestCheckoutAfterRefund','checkout.session.completed','acct_TestSeller',false,'cs_test_TestSession',repeat('3',64),now()+interval '2 hours','${JSON.stringify({attempt_id:prepared.attempt_id,payment_intent_id:'pi_TestIntent'})}'::jsonb) value`)).rows[0].value);
  assert.equal(checkoutAfterRefund.note,'checkout_state_preserved');
  const successAfterRefund=json((await db.query(`select public.apply_market_stripe_event('evt_TestSuccessAfterRefund','payment_intent.succeeded','acct_TestSeller',false,'pi_TestIntent',repeat('4',64),now()+interval '3 hours','${JSON.stringify({attempt_id:prepared.attempt_id,amount_cents:10500,currency:'eur',charge_id:'ch_TestCharge'})}'::jsonb) value`)).rows[0].value);
  assert.equal(successAfterRefund.note,'post_payment_state_preserved');
  const failureAfterRefund=json((await db.query(`select public.apply_market_stripe_event('evt_TestFailureAfterRefund','payment_intent.payment_failed','acct_TestSeller',false,'pi_TestIntent',repeat('5',64),now()+interval '4 hours','${JSON.stringify({attempt_id:prepared.attempt_id})}'::jsonb) value`)).rows[0].value);
  assert.equal(failureAfterRefund.note,'post_payment_state_preserved');
  const repeatedRefund=json((await db.query(`select public.apply_market_stripe_event('evt_TestRefundAgain','charge.refunded','acct_TestSeller',false,'ch_TestCharge',repeat('6',64),now()+interval '5 hours','${JSON.stringify({attempt_id:prepared.attempt_id,amount_cents:10500,currency:'eur',refund_id:'re_TestRefund'})}'::jsonb) value`)).rows[0].value);
  assert.deepEqual({status:repeatedRefund.status,note:repeatedRefund.note},{status:'ignored',note:'refund_already_confirmed'});
  await db.exec('reset role;');
  assert.equal(Number((await db.query('select count(*) count from dv_market_private.tax_test_corrections')).rows[0].count),2);
  assert.deepEqual((await db.query(`select state,paid_cents,refunded_cents from dv_market_private.market_payment_attempts where id='${prepared.attempt_id}'`)).rows[0],{state:'refunded',paid_cents:10500,refunded_cents:10500});
  assert.equal((await db.query(`select payment_status from public.market_orders where id='${ORDER}'`)).rows[0].payment_status,'refunded');
  await db.exec('set role service_role;');
