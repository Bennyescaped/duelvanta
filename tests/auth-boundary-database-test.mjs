...
const db=new PGlite();
const H='10000000-0000-4000-8000-000000000001',G='10000000-0000-4000-8000-000000000002',X='10000000-0000-4000-8000-000000000003',B='10000000-0000-4000-8000-000000000004',M='20000000-0000-4000-8000-000000000001',S='30000000-0000-4000-8000-000000000001';
...
try{
 ...
 insert into public.profiles(id) values ('${H}'),('${G}'),('${X}'),('${B}');
 ...
 await db.exec(await read('supabase/migrations/20260918123500_battle_beta_account_status_v1.sql'));
 await db.exec(`update public.profiles set account_status='beta' where id in ('${H}','${B}'); update public.profiles set account_status='suspended' where id='${X}';`);
 ...
 await claim(B);assert.ok((await db.query(`select public.create_battle_match_v2('pokemon','public',null,'de','casual') as id`)).rows[0].id,'beta account can create BATTLE match');
 await claim(X);await blocked(`select public.set_battle_ready('${M}',true)`,/battle_account_unavailable/);await blocked(`select public.start_battle_match('${M}')`,/battle_account_unavailable/);
 ...
 // B06's eight success-returning stub RPCs are superseded by the mandatory native
 // security-privilege-mfa-database-test.mjs against actual versioned implementations.
 console.log('PASS: auth boundary allows beta/active BATTLE accounts, blocks suspended/restricted/null access, and keeps owner/profile update boundaries');
