...
const {authenticatedUser,json,required,rpc,stripeMode,stripeV2Request}=require('./market-stripe-lib');

module.exports=async function handler(req,res){
  if(req.method!=='POST')return json(res,405,{error:'method_not_allowed'});
  let mode;try{mode=stripeMode('onboarding')}catch(error){const code=String(error.message||error);return json(res,code==='stripe_sandbox_disabled'?409:503,{error:code})}
  ...
  try{
    ...
    if(link.livemode!==mode.liveMode||link.object!=='v2.core.account_link'||!/^https:\/\/(connect|accounts)\.stripe\.com\//.test(link.url||''))throw new Error('stripe_account_link_invalid');
    ...
    return json(res,200,{status:mode.liveMode?'stripe_live_onboarding_ready':'stripe_test_onboarding_ready',onboarding_url:link.url,live_mode:mode.liveMode});
