...
## Schritt 9B – projektgebundene Betreiberidentität (Kandidatenkorrektur)

Das echte Staging-Betreiberkonto besitzt eine neue, durch Supabase regulär erzeugte UUID. Die bisherige feste UUID in is_duelvanta_owner, protect_duelvanta_owner_profile und prevent_duelvanta_owner_delete ist deshalb kein umgebungsübergreifendes Soll. Keine Auth-ID kopieren/manipulieren, keine Schutzfunktion abschalten.

Der erste SQL-Kandidat bindet den bereits administrativ freigegebenen, E-Mail-bestätigten info@duelvanta.de-Owner einmalig in dv_v16_private.operator_identity_v1. Verwendung des bestehenden privaten Schemas vermeidet eine weitere exponierte API. Singleton, eindeutige Benutzer-ID, Auth-FK mit RESTRICT, RLS ohne Browserpolicy und keinerlei Browser-/Service-Tabellenrechte. Keine Neuzuordnung bei erneuter Migration oder E-Mail-Übertragung. Keine Bindung bedeutet owner=false; kein Legacy-UUID-Fallback. Datenbankadministration bleibt der einzige Provisionierungsweg. Bindungsdaten sind projektspezifische Konfiguration, kein Schemafingerprint.

Die Identitätsfunktion verlangt gebundene ID plus aktuell bestätigte Betreiber-E-Mail. Update-/Löschschutz benutzt die feste Bindung auch bei zwischenzeitlich geänderter Auth-E-Mail. Privilegierte Handlungen benötigen weiterhin separat AAL2, aktive Session und fachliche Rolle. Profilstatus beta wird ausschließlich zum regulären Bootstrap akzeptiert; bestehende Geschäftsprüfungen, die active verlangen (z.B. Scanner), bleiben erhalten. Nach gesicherter Bindung ist das echte Betreiberprofil kontrolliert auf active zu setzen.

Isolierte Tests verwenden bewusst eine andere UUID als den früheren Production-Wert; prüfen fehlende Bindung, unbestätigte/übertragene E-Mail, unveränderliche Bindung bei Wiederanwendung, Owner-Update-/Löschschutz, Auth-FK und verweigerte Browser-/Servicezugriffe. Die vorhandene Rollen-/MFA-/Geschäftsregression läuft damit vollständig erneut. Readiness wird offline aus diesem bewusst geänderten Soll erzeugt, nicht aus dem Live-Katalog. Live-Anwendung weiterhin erst nach aktueller Sicherung und vollständigem CI-PASS.
