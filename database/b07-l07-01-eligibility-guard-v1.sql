...
create table if not exists dv_market_private.trade_user_eligibility (
  ...
  check (date_of_birth >= current_date - interval '120 years')
);
...
-- evidence for selling eligibility, but never infer the separate private-buyer declaration.
insert into dv_market_private.trade_user_eligibility(
  user_id,date_of_birth,residence_country_code,private_buyer_confirmed,confirmed_at,updated_at
)
select p.seller_id,p.date_of_birth,'DE',false,coalesce(a.terms_accepted_at,a.created_at,now()),now()
...
create or replace function public.confirm_my_market_trade_eligibility(
  ...
  p_confirm_private_buyer boolean
) returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public, dv_market_private
...
begin
  ...
  if coalesce(p_confirm_private_buyer,false) is not true then
    raise exception 'buyer_must_confirm_private_consumer';
  end if;

  insert into dv_market_private.trade_user_eligibility(
    user_id,date_of_birth,residence_country_code,private_buyer_confirmed,confirmed_at,updated_at
  ) values(v_uid,p_date_of_birth,'DE',true,now(),now())
...
begin
  ...
  if v_row.user_id is null
     ...
     or v_row.date_of_birth > current_date - interval '18 years' then
...
comment on table dv_market_private.trade_user_eligibility is
  'B07 Release-1 technical eligibility only: registered authenticated user, 18+, DE residence; private-buyer declaration is separate from seller role.';
