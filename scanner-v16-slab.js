(()=>{
  'use strict';
  const root=globalThis,OPENAI_MODEL='gpt-5.4-mini',XIMILAR_MODEL='ximilar-collectibles-v2-slab-id';
  function fields(value){
    ...
    if(!/^[A-Z][A-Z0-9 .&-]{1,39}$/.test(company)||company==='OTHER'||!Number.isFinite(grade)||grade<1||grade>10||!/^\d+(?:\.\d+)?$/.test(rawGrade)||(cert&&!/^[A-Za-z0-9-]{3,60}$/.test(cert)))return null;
    return{grading_company:company==='BECKETT'?'BGS':['PSA','BGS','CGC','ACE'].includes(company)?company:'OTHER',grade,cert_number:cert||null};
  }
  function canSave(row){return !row.slab||(row.slabConfirmed===true&&row.manualConfirmed===true&&!!fields(row.slab))}
  function confirm(row,value){const parsed=fields(value);if(!row?.slab||!parsed)throw new Error('Grading-Firma und Grade (1–10) vollständig prüfen.');row.slab={...row.slab,company:String(value.company).trim().toUpperCase(),grade:String(parsed.grade),certificateNumber:parsed.cert_number||''};row.slabConfirmed=true;return parsed}
  function rect(source,proposal){
    ...
    if(!r&&proposal.model===OPENAI_MODEL){const area={x:w*.10,y:h*.20,w:w*.80,h:h*.76},ratio=63/88;let rw=area.w,rh=area.h,x=area.x,y=area.y;if(rw/rh>ratio){const next=rh*ratio;x+=(rw-next)/2;rw=next}else{const next=rw/ratio;y+=(rh-next)/2;rh=next}return{x,y,w:rw,h:rh}}
  ...
  async function analyze(source,opts,base){
    ...
    if(![OPENAI_MODEL,XIMILAR_MODEL].includes(p.model)||p.selectedTcg!==opts.tcg||p.status==='tcg_conflict')throw new Error('Slab und gewähltes Kartenspiel stimmen nicht überein.');
    if(!['single','continuous'].includes(opts.mode||'single'))throw new Error('Ein Slab pro Aufnahme.');
    const observed=p.observed||{},slab=p.slab||{company:observed.grading_company,grade:observed.grade,certificateNumber:observed.certificate_number,subgrades:observed.subgrades||[]};
    if(p.status==='slab_label_missing'||!slab.company||!slab.grade)throw new Error('Slab-Label nicht erkannt.');
    const core=root.DV_SCAN_V16_CORE,crop=core.canvasFrom(source,rect(source,p),1000),photo=core.canvasFrom(source,null,1200);
    const localOpts={...opts,slabObservation:undefined,providerObservation:p.model===OPENAI_MODEL?p:undefined,sourcePrepared:true};
    let pack=await base(crop,localOpts);
    const hint=labelIdentifier(slab,opts.tcg)||root.DV_SCAN_V16_RECOVERY?.parse?.(observed.printed_code,opts.tcg)||null;
    ...
    for(const row of pack.results){
      row.slab={...slab};row.slabConfirmed=false;row.capturePhoto=photo;row.status='review';row.selected=false;row.manualConfirmed=false;
      row.captureKind='slab';row.providerEvidence={...(row.providerEvidence||{}),provider:p.model===OPENAI_MODEL?'openai':'ximilar',model:p.model,reviewRequired:true,elapsedMs:p.elapsedMs,printingId:null,identifierConflict:!!row.labelIdentifierConflict};
  ...
  root.DV_SCAN_V16_SLAB={fields,canSave,confirm,rect,labelIdentifier,analyze,model:OPENAI_MODEL};
