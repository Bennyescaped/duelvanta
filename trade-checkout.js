/* Checkout V1.2: immutable C2C/B2C review; reuses the existing authenticated client. */
(() => {
  'use strict';
  ...
  function renderServerReview(data) {
    ...
    byId('dvCheckoutParty').innerHTML=`<div class="dvCheckoutRole"><b>${data.contract_classification==='b2c'?'B2C · GEWERBLICHER VERKAUF':'C2C · PRIVATVERKAUF'}</b><span>${esc(seller.role_label||'Verkäufer')}</span></div><div><strong>Vertragspartner</strong><br>${esc(business)}${seller.legal_name&&seller.business_name?` · ${esc(seller.legal_name)}`:''}<br>${esc(address(seller))}${seller.public_email?`<br>${esc(seller.public_email)}`:''}</div><div class="dvCheckoutLegal">${data.contract_classification==='b2c'?'Für diesen gewerblichen Verkauf werden die Verbraucherinformationen und die Verkäuferrolle im Vertragsnachweis festgehalten.':'Privatverkauf: DUELVANTA ist Vermittler und nicht Verkäufer der Ware. Die besonderen B2C-Unternehmerpflichten werden nicht als Leistung von DUELVANTA dargestellt.'}</div>`;
  ...
  function scheduleReview(quantity) {
    const request=++reviewRequest,listing=current;
    clearTimeout(reviewTimer);review=null;byId('dvBuyNow').disabled=true;
    byId('dvCheckoutParty').innerHTML='<div class="msg">Verkäuferrolle und Vertragsdaten werden serverseitig geprüft …</div>';
    reviewTimer=setTimeout(async()=>{
      const response=await db.rpc('review_market_checkout',{p_listing_id:listing.id,p_quantity:quantity});
      if(request!==reviewRequest||current?.id!==listing.id||selectedQuantity()!==quantity)return;
      if(response.error){byId('dvCheckoutParty').innerHTML='';byId('dvBuyMsg').textContent=response.error.message;return}
      review=response.data;byId('dvBuyMsg').textContent='';renderServerReview(review);byId('dvBuyNow').disabled=busy;
    },80);
  }
  function refresh() {
    ...
    review=null;byId('dvBuyNow').disabled=true;
  ...
  function changeQuantity(delta) {
    ...
    input.value=String(Math.max(min,Math.min(max,value+delta)));input.dispatchEvent(new Event('input',{bubbles:true}));
  }
  function purchaseRequest(quantity) {
    const key=`dv-checkout:${user.id}:${current.id}`,signature=`${current.updated_at}:${quantity}:${review.checkout_hash}`;let saved=null;
    ...
    if(retry?.signature===signature)saved=retry;retry={...saved,key};return retry;
  }
  async function buy() {
    ...
    byId('dvBuyMsg').textContent='Verbindliche Bestellung und Vertragsnachweis werden angelegt …';let result;
    try{
      const response=await db.rpc('buy_market_listing_v3',{p_listing_id:listing.id,p_quantity:quantity,p_request_id:request.id,p_expected_updated_at:listing.updated_at,p_checkout_hash:checkoutReview.checkout_hash});
      if(response.error)throw response.error;result=response.data;
    }catch(error){
      byId('dvBuyMsg').textContent=error.message||'Keine Bestätigung erhalten. Du kannst dieselbe Anfrage sicher wiederholen.';busy=false;byId('dvBuyQty').disabled=false;
      if(String(error.message||'').includes('checkout_review_changed'))refresh();else{review=checkoutReview;byId('dvBuyNow').disabled=false;byId('dvBuyMinus').disabled=false;byId('dvBuyPlus').disabled=false}return;
    }
    busy=false;try{sessionStorage.removeItem(request.key)}catch(_){}retry=null;byId('dvBuyMsg').textContent='';
    byId('dvBuyBody').innerHTML=`<div class="dvCheckoutSuccess"><div class="dvCheckoutSuccessMark">✓</div><b>BESTELLUNG VERBINDLICH ANGELEGT</b><p>${quantity} × ${esc(listing.card_name)}</p><p>Warenwert: ${cash(result.item_total)}</p><div class="msg">Der unveränderbare ${esc(String(result.contract_classification||'').toUpperCase())}-Vertragsnachweis ist unter Bestellungen abrufbar. Es wurde keine Onlinezahlung ausgeführt.</div></div><div class="actions"><button id="dvBuyGoOrder" class="btn gold">BESTELLUNG & NACHWEIS ÖFFNEN</button><button id="dvBuyContinue" class="btn ghost">WEITER EINKAUFEN</button></div>`;
    byId('dvBuyContinue').onclick=()=>byId('dvBuyDialog').close();byId('dvBuyGoOrder').onclick=()=>{byId('dvBuyDialog').close();window.DV_TRADE_ORDERS.open(result.order_id)};
  ...
  function install() {
    ...
    const dialog=document.createElement('dialog');dialog.id='dvBuyDialog';dialog.innerHTML='<div class="modal"><div class="modalHead"><h2>Verbindlichen Kauf prüfen</h2><button id="dvBuyClose" type="button" class="close" aria-label="Kaufdialog schließen">✕</button></div><div id="dvBuyBody"></div><div id="dvBuyMsg" class="msg" aria-live="polite"></div></div>';document.body.appendChild(dialog);
    ...
    new MutationObserver(decorate).observe(byId('grid'),{childList:true});decorate();window.DV_TRADE_CHECKOUT={version:'1.2',priceFor};return true;
  }
