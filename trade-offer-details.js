/* Participant-only negotiated price offers. Seller acceptance forms the contract. */
(() => {
  'use strict';
  const labels={pending:'OFFEN',accepted:'ANGENOMMEN · VERTRAG GESCHLOSSEN',declined:'ABGELEHNT',withdrawn:'ZURÜCKGEZOGEN',expired:'ABGELAUFEN',cancelled:'STORNIERT'};
  ...
  renderOffers=async function(){
    ...
    $('grid').innerHTML=(data||[]).map(o=>{
      const incoming=o.seller_id===user.id,snapshot=o.listing_snapshot||{},review=o.contract_review_snapshot||null,validReview=review?.snapshot_version==='price-offer-contract-v1',qty=Number(o.requested_quantity||1),amount=Number(o.amount||0);
      ...
      if(o.status==='pending')actions=incoming
        ?(validReview?`<button class="btn gold" data-accept-offer="${o.id}">VERBINDLICH ANNEHMEN</button><button class="btn danger" data-decline-offer="${o.id}">ABLEHNEN</button>`:`<button class="btn danger" data-decline-offer="${o.id}">ABLEHNEN</button>`)
      ...
      if(o.status==='accepted'&&!o.order_id)actions='<span class="msg">Der Vertrag wurde angenommen; die Order-Zuordnung wird noch synchronisiert. Bitte neu laden.</span>';
      ...
      const frozen=validReview?`<div class="dvOfferContract"><b>VERTRAGSDATEN DES PREISANGEBOTS</b><br>Produktangebot: ${money(review.offered_goods_total)} · Versand: ${money(review.shipping_cost)} · <strong>Gesamtpreis: ${money(review.total_price)}</strong><br>${esc(review.offer_notice||'Der Vertrag entsteht erst durch Annahme des Verkäufers.')}</div>`:(o.status==='pending'?'<div class="msg">Dieses ältere Angebot enthält keinen vollständigen Vertrags-Snapshot und kann im neuen Modell nicht angenommen werden.</div>':'');
      const contract=o.status==='accepted'?'<div class="dvOfferContract">VERTRAG GESCHLOSSEN · Der Verkäufer hat das verbindliche Preisangebot angenommen. Ein weiterer Checkout ist für den Vertragsschluss nicht erforderlich.</div>':'';
      const state=o.status==='cancelled'?'Die zugehörige Order wurde storniert.':o.order_id?'Zahlung, Versand und Nachweis laufen über die Order.':o.status==='pending'?'Noch kein Vertrag.':o.status==='accepted'?'Vertrag geschlossen; Order wird zugeordnet.':'Kein Vertrag geschlossen.';
      return `<article class="listing dvOfferDetail"><div class="body"><div class="eyebrow">${incoming?'EINGEGANGEN':'GESENDET'} · ${esc(labels[o.status]||o.status)}</div><h3>${esc(snapshot.card_name||'Produkt')}</h3><div class="meta">${incoming?'Käufer':'Verkäufer'}: ${person}<br>${esc([snapshot.set_name,snapshot.language].filter(Boolean).join(' · '))}</div><div class="dvOfferQuantity"><b>${qty} × ${esc(type)}</b></div><div class="dvOfferPrices"><div><span>Inseratspreis für diese Menge</span><strong>${reference===null?'Nicht gespeichert':money(reference)}</strong><small>${reference===null?'':`${qty} × ${money(o.listed_unit_price_snapshot)} · Mengenrabatt berücksichtigt`}</small></div><div><span>${incoming?'Gebotener Produktgesamtpreis':'Dein Produktgesamtpreis'}</span><strong>${money(amount)}</strong><small>${qty} Stück · ${amount*100%qty===0?'':'Ø '}${money(unit)} pro Stück</small></div></div>${comparison}${frozen}${contract}<div class="msg">${state}${validReview?` Versand im Angebot: ${money(review.shipping_cost)}.`:(snapshot.shipping_cost!=null?` Laut Anzeige: ${money(snapshot.shipping_cost)}.`:'')}</div><div class="dvOfferMessage">${esc(o.message||'Keine Nachricht')}</div><div class="actions">${actions}</div></div></article>`;
