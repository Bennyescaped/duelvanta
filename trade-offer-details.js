/* Participant-only offer overview. Accepted proposals reserve checkout for 2h; acceptance is not a contract. */
(() => {
  'use strict';
  const labels = {pending:'OFFEN',accepted:'ANGENOMMEN · RESERVIERT',declined:'ABGELEHNT',withdrawn:'ZURÜCKGEZOGEN',expired:'ABGELAUFEN',cancelled:'STORNIERT'};
  ...
  renderOffers = async function () {
    ...
    $('grid').innerHTML = (data || []).map(o => {
      ...
      if (o.status === 'accepted' && !o.order_id) actions = incoming
        ? '<span class="msg">Der Käufer hat bis zum Ablauf der Reservierung exklusiven Checkout-Zugriff.</span>'
        : `<button class="btn gold" data-checkout-offer="${o.id}">ZUM VERBINDLICHEN CHECKOUT</button>`;
      ...
      let reservation='';
      if(o.status==='accepted'&&!o.order_id&&o.reservation_expires_at){const end=new Date(o.reservation_expires_at);reservation=`<div class="dvOfferReservation">2-STUNDEN-RESERVIERUNG · bis ${end.toLocaleString('de-DE',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}. Die Annahme selbst ist noch kein Kaufvertrag.</div>`}
      return `<article class="listing dvOfferDetail"><div class="body"><div class="eyebrow">${incoming?'EINGEGANGEN':'GESENDET'} · ${esc(labels[o.status] || o.status)}</div><h3>${esc(snapshot.card_name || 'Produkt')}</h3><div class="meta">${incoming?'Käufer':'Verkäufer'}: ${person}<br>${esc([snapshot.set_name,snapshot.language].filter(Boolean).join(' · '))}</div><div class="dvOfferQuantity"><b>${qty} × ${esc(type)}</b></div><div class="dvOfferPrices"><div><span>Inseratspreis für diese Menge</span><strong>${reference===null?'Nicht gespeichert':money(reference)}</strong><small>${reference===null?'':`${qty} × ${money(o.listed_unit_price_snapshot)} · Mengenrabatt berücksichtigt`}</small></div><div><span>${incoming?'Gebotener Produktgesamtpreis':'Dein Produktgesamtpreis'}</span><strong>${money(amount)}</strong><small>${qty} Stück · ${amount*100%qty===0?'':'Ø '}${money(unit)} pro Stück</small></div></div>${comparison}${reservation}<div class="msg">${o.status==='cancelled'?'Die zugehörige Order wurde storniert.':o.order_id?'Versand separat über die Order.':'Noch keine Bestellung / kein Vertrag.'}${snapshot.shipping_cost!=null?` Laut Anzeige: ${money(snapshot.shipping_cost)}.`:''}</div><div class="dvOfferMessage">${esc(o.message || 'Keine Nachricht')}</div><div class="actions">${actions}</div></div></article>`;
